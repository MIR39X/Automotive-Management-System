...
$errors = [];
$customer_id = $vehicle_id = $employee_id = null;
$date_in = date('Y-m-d');
$status = 'open';
$notes = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    $stmt = $pdo->prepare("
      INSERT INTO jobcard (customer_id, vehicle_id, employee_id, date_in, status, notes)
      VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$customer_id, $vehicle_id, $employee_id, $date_in, $status, $notes]);
...
<div class="jobcard-hero">
  <div>
    <h2>New Job Card</h2>
    <p>Create a workshop job card for a customer's vehicle.</p>
  </div>
